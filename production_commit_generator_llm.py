...
class ProductionCommitFormatter:
    ...
    def format_commit_message(self, change_type: str, summary: str, scope: str = None) -> str:
        ...
        task = Task(
            description=f"""
            Format a conventional commit message with the following details:
            ...
            2. Uses appropriate type (feat, fix, docs, style, refactor, test, chore, build, ci)
            3. Includes scope if relevant
            4. Has a clear, concise description
            5. Is under 50 characters for the subject line
            
            Return only the formatted commit message, no additional text.
            """,
            agent=self.agent,
            expected_output="Formatted conventional commit message"
        )
